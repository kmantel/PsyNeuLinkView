  ...
  set_file_loader() {
    ...
    this.file_loader.onchange = e => {
      ...
      if(e.path[0].files.length > 0) {
        ...
        var uri = 'http://127.0.0.1:5000/api/v1/resources/script?filepath=' + filepath
        self.get_request(uri,
          async function () {
            var chosen_composition = await self.choose_composition()
            console.log(chosen_composition)
            self.set_composition(chosen_composition)
          }
        )
      }
    }
